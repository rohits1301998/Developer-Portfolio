    ...
    this.menuItems = (
      <div className='mobile-menus'>
        <ScrollSpy duration={2000} offsetTop={48} className="active-tab">
          {this.tabs.map((t) => (
            <a onClick={ this.toggleNavMenus.bind(this) } key={t.id} className="nav-tab text-dark mx-3 text-uppercase" ref={React.createRef()} href={`#` + t.id}>
              {t.label}
            </a>
          ))}
        </ScrollSpy>
        ...
        <nav className="bg-white">
          <div className="row align-items-center h-100">
            ...
            <div className='col nav-container-sm d-none justify-content-end align-items-center'>
              <button onClick={ this.toggleNavMenus.bind(this) } className='mx-2 bg-white border-0'>
